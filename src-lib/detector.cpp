...
float validate_detector_map(char* datacfg, char* cfgfile, char* weightfile, float conf_thresh, const float iou_thresh, const int map_points, int letter_box, network* existing_net)
{
	...
	struct box_prob
	{
		...
		int class_id;			// matching class index
		int image_index;		// reference to the image
		int truth_flag;			// Does this match a truth?
		int unique_truth_index;	// Reference to the global truth index - the index of the matching truth label, across ALL files
	};
			...
			for (int i = 0; i < nboxes; ++i)
			{
				for (int class_id = 0; class_id < classes; ++class_id)
				{
					...
					if (prob > 0.0f)
					{
						...
						detections[detections_count - 1].prob = prob;
						...
						if (prob > conf_thresh)
						{
							// See if any previous detection matched this truth
							int found = 0;
							for (int z = checkpoint_detections_count; z < detections_count - 1; ++z)
							{
								if (detections[z].unique_truth_index == truth_index /*&& truth_index > -1*/)
								{
									found = 1;
									break;
								}
	...
	std::sort(detections, detections + detections_count,
		...
		{
			const auto diff = lhs.prob - rhs.prob;
			if (diff < 0.0f)
			{
				return false;
			}
	...
	for (int rank = 0; rank < detections_count; ++rank)
	{
		...
		pr[d.class_id][rank].prob = d.prob;
	...
	printf("\n for conf_thresh = %1.2f, precision = %1.2f, recall = %1.2f, F1-score = %1.2f \n", conf_thresh, cur_precision, cur_recall, f1_score);

	printf(" for conf_thresh = %0.2f, TP = %d, FP = %d, FN = %d, average IoU = %2.2f %% \n", conf_thresh, tp_for_thresh, fp_for_thresh, unique_truth_count - tp_for_thresh, avg_iou * 100);
